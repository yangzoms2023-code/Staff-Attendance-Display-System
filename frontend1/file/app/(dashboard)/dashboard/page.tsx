...
export default function DashboardPage() {
  ...
  //check user session here whether it reached or not console.log('',)
  ...
  const getEmployeeName = (employeeId: string) => {
    ...
    if (!employee) employee = employees.find(e => e.id === employeeId || e.employeeId === employeeId)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatsCard icon={<Users className="h-6 w-6 text-slate-600" />} label="Total Employees" value={employees.length} />
        <StatsCard icon={<UserCheck className="h-6 w-6 text-emerald-600" />} label="Present Today" value={totalPresent} />
        <StatsCard icon={<UserX className="h-6 w-6 text-rose-600" />} label="Absent Today" value={totalAbsent} />
        <StatsCard icon={<CalendarX className="h-6 w-6 text-purple-600" />} label="On Leave" value={stats?.onLeave ?? 0} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-2">
            ...
            <CardDescription>Attendance trend for the last 7 days (Present, Absent, Leave)</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[320px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={weeklyData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <YAxis tick={{ fontSize: 12 }} axisLine={false} tickLine={false} domain={[0, 'dataMax + 2']} allowDecimals={false} />
                  ...
                  <Bar dataKey="absent" name="Absent" fill="#ef4444" radius={[6, 6, 0, 0]} maxBarSize={40} />
                  <Bar dataKey="leave" name="Leave" fill="#8b5cf6" radius={[6, 6, 0, 0]} maxBarSize={40} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="border-0 shadow-sm">
          ...
          <CardContent>
            <div className="h-[320px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={3} dataKey="value" labelLine={false} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
            ...
            <div className="mt-4 text-center text-xs text-muted-foreground">
              Total accounted: {pieData.reduce((sum, item) => sum + item.value, 0)} / {employees.length} employees
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-2">
          ...
          <CardDescription>Current status of all staff members</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {recentAttendance.length === 0 && employees.length > 0 ? (
              ...
              employees.slice(0, 10).map((employee) => {
                const record = recentAttendance.find(a => a.employeeId === employee.id)
                const isOut = currentlyOutStaff.some(o => o.employeeId === employee.id)
                ...
                if (record?.status === "Leave") {
                  ...
                  displayStatus = record.checkOut ? "Checked Out" : "In Office"
                  statusColor = record.checkOut ? "bg-blue-100 text-blue-700" : "bg-emerald-100 text-emerald-700"
                }

                return (
                  <div key={employee.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-3">
                    ...
                    <Badge className={cn("rounded-full px-3 py-1 text-xs font-medium", statusColor)}>
                      {displayStatus}
                    </Badge>
                  </div>
                )
