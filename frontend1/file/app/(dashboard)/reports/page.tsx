	...
	const overallStats = {
		...
		avgAttendance:
			reportData.length > 0
				...
				: 0,
		totalPresent: reportData.reduce(
			(sum, r) => sum + r.presentDays + r.lateDays,
			0,
		),
			...
			<div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
				<div className="space-y-1">
					...
					<p className="text-sm sm:text-base text-slate-500">
						Generate monthly attendance reports (Monday-Friday
						working days only)
					</p>
				</div>
				<Button
          ...
          shadow-sm h-9 sm:h-10 px-4 sm:px-5 shrink-0 self-start sm:self-auto w-full sm:w-auto transition-colors"
			...
			<Tabs defaultValue="employee" className="space-y-4">
				<TabsList className="bg-slate-100 p-1 rounded-lg">
					<TabsTrigger
						value="employee"
						className="text-sm data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm"
					>
						By Employee
					</TabsTrigger>
					<TabsTrigger
